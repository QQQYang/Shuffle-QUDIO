...
for i in range(2):
    ...
    ax[i].set_xlabel('Number of local nodes (K)', font, fontsize=fontsize)
    ax[i].grid(True)
...
ax[1].set_title('Speedup to accuracy', font, fontsize=fontsize)
...
leg = fig.legend(handles, labels, loc=(0.15, 0.65), fontsize=fontsize, frameon=True, ncol=3)
leg.get_frame().set_linewidth(2)
leg.get_frame().set_edgecolor('k')

plt.tight_layout()
plt.savefig('figure/speedup_vqe_shuffle_single_col.pdf')
plt.savefig('figure/speedup_vqe_shuffle_single_col.png')
plt.show()